...
def assign_evidence_type(summary: pd.DataFrame, type_column_name: str = 'new_type'):
    ...
    msms_in_rt_window = ((summary['Retention time'] >= summary['Calibrated retention time start'] - summary[
        'Retention time calibration']) & (
                                 summary['Retention time'] <= summary['Calibrated retention time finish'] - summary[
                             'Retention time calibration']))
...
def match_precursor_grouped(msms_scan: pd.Series, allpeptides_grouped: pd.core.groupby.DataFrameGroupBy,
                            group_key: List[str]):
